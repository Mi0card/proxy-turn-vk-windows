...
- **Системный прокси** — чекбокс в шапке, перенаправляет WinINET-трафик (Chrome, Edge, Office) через туннель по правилам маршрутизации. Отдельный HTTP-прокси на случайном порту, без auth, без прав администратора
...
При включении поднимается отдельный HTTP-прокси на случайном порту (`127.0.0.1:<port>`, без аутентификации) и прописывается в настройках Windows (WinINET, ветка `HKCU`). Системный прокси подчиняется тем же правилам маршрутизации, что и SOCKS5/HTTP: по умолчанию трафик идёт через туннель, а правила могут направить его напрямую (`direct`) или заблокировать (`block`). Соединения системного прокси видны на вкладке «Подключения».

- Для политики по умолчанию (`proxy`) требуется активный туннель. Без него такие запросы браузеров отклоняются (502).
...
Правила действуют для SOCKS5, HTTP-прокси и системного прокси. Соединения показываются на вкладке «Подключения» в формате `→ хост:порт - приложение - [время, политика]`, где приложение определяется по процессу-владельцу соединения (на Windows/macOS), например `→ 8.8.8.8:2345 - chrome.exe - [10391ms, proxy]`. Если определить не удалось — `—`; заблокированные соединения помечаются `[block]`.
