...
- Авто-загрузка WireGuard конфига, статус-бар с пингом через туннель, автоперезапуск при обрыве соединения (сон, смена сети, таймаут всех воркеров — с кулдауном)
- Светлая / тёмная тема
...
Вкладка **Маршрутизация** позволяет направлять трафик по правилам (аналогично приложению Throne). Есть два вида правил.

### Встроенные правила (без скачивания)

Эти правила хранятся в конфиге и работают сразу, не требуя загрузки дата-файлов:

- `domain:<домен>` — точное совпадение домена (например `domain:example.com`)
- `domain-suffix:<домен>` — домен и все поддомены (например `domain-suffix:sobaka.com`)
- `keyword:<текст>` — подстрока в домене (например `keyword:youtube`)
- `regex:<шаблон>` — регулярное выражение по домену (например `regex:\.example\.com$`)
- `cidr:<подсеть>` — IP-подсеть (например `cidr:10.0.0.0/8`)
- `ip:<адрес>` — конкретный IP (например `ip:1.2.3.4`)

### Правила из дата-файлов

Правило задаётся в формате `ruleset:<тип>-<группа>` и ссылается на группу из скачанных дата-файлов:
...
При вводе правил можно не писать префикс `ruleset:` — он добавляется автоматически (для встроенных правил префикс обязателен).
